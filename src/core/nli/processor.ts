...
export class NaturalLanguageProcessor {
  ...
  updateContext(commandId: string, result: ExecutionResult): void {
    // Update command history
    this.context.previousCommands.push(commandId);

    // Trim history if too long
    if (this.context.previousCommands.length > 10) {
      const removedId = this.context.previousCommands.shift();
      if (removedId) {
        this.commandHistory.delete(removedId);
      }
    }
